...
void prog()
{
    ...
    std::int64_t prev_n = a.front(), prev_f = -a.front();
    for (auto i : boost::irange(1ul, n - 1)) {
        std::int64_t n = 0, f = 0;
        n = std::max(prev_n + a[i], prev_f - a[i]);
        f = std::max(prev_n - a[i], prev_f + a[i]);
        prev_n = n;
        prev_f = f;
    }
    out(std::max(prev_n + a.back(), prev_f - a.back()));
}
