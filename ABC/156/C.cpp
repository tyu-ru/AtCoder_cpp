...
void prog()
{
    ...
    int n = in.read<int>();
    auto x = in.read<int>(n);
    auto sum = std::accumulate(std::begin(x), std::end(x), 0);
    int g = sum / n;
    auto cost = [&](int p) {
        return std::accumulate(x.begin(), x.end(), 0, [&](int a, int b) { return a + (b - p) * (b - p); });
    };

    out(std::min(cost(g), cost(g + 1)));
}
