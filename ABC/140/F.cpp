...
int main()
{
    ...
    int N = in.read<int>();
    StaticVector<int, (1 << 18)> S(in, 1 << N);
    std::sort(S.begin(), S.end(), std::greater<>());

    std::vector<int> birth, parent;
    birth.push_back(S.front());
    S.front() = 0;

    for (int i : boost_irange(N)) {
        parent = birth;
        auto it = S.begin();
        for (int x : parent) {
            it = std::find_if(it, S.end(), [&](auto y) { return y != 0 && y < x; });
            if (it == S.end()) {
                out << "No";
                return 0;
            }
        ...
        std::sort(birth.begin(), birth.end(), std::greater<>());
    }
