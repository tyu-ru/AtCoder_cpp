...
{
    // static std::map<std::pair<ModInt<>, ModInt<>>, ModInt<>> buf;
    // {
    //     auto it = buf.find({n, r});
    //     if (it != buf.end()) return it->second;
    // }
    // {
    //     auto it = buf
    // }
...
void prog()
{
    ...
    {
        auto t = in.read<int64_t>(n);
        std::sort(std::begin(t), std::end(t));
        for (auto i : boost::irange<std::uint64_t>(0, n)) {
            // a[i + 1] = a[i] + (t[i] + 1000000000);
            a[i + 1] = a[i] + (t[i] < 0 ? ModInt<>::mod + t[i] : t[i]);
        }
    }
    // out.print(a);
    ModInt<> res = 0;
    ModInt<> cmb = 1;
    auto& inv = ModInt<>::inv(n + 1);
    for (auto i : boost::irange<uint64_t>(k, n + 1)) {
        // auto c = binomial(i - 2, k - 2);
        // out(n - i + 1, i - 1);
        res += cmb * ((a[n] - a[i - 1]) - a[n - i + 1]);
        cmb *= i - 1;
        cmb *= inv[i - k + 1];
    }
    out(res.value());
