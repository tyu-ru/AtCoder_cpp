...
int main()
{
    ...
    std::uint64_t res = 0;
    for (int i : boost::irange(0, N - 2)) {
        for (int j : boost::irange(i + 1, N - 1)) {
            res += std::distance(std::upper_bound(L.begin() + j + 1, L.end(), L[j] - L[i]),
                                 std::lower_bound(L.begin() + j + 1, L.end(), L[i] + L[j]));
        }
    }
    out << res;
