...
int main()
{
    ...
    auto res = std::numeric_limits<std::uint64_t>::max();
    for (auto i : boost::irange(std::uint64_t{1}, static_cast<std::uint64_t>(std::ceil(std::sqrt(N)) + 1))) {
        if (N % i) continue;
        res = std::min(res, (N / i) + i - 2);
    }
    out << res;
