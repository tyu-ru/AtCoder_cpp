...
void prog()
{
    ...
    for (int x : read<int>(m)) {
        mp[d[x]] = std::max(mp[d[x]], x);
    }
    ...
    for (auto i : boost::irange(1, n + 1)) {
        for (auto j : mp) {
            ...
            dp[i] = std::max(dp[i], dp[i - j.first] + 1);
        }
    }

    std::map<int, int, std::greater<>> mp2;
    for (auto p : mp) {
        mp2.emplace(p.second, p.first);
    }
    for (int i = n, k = dp[n]; 0 < k;) {
        for (auto j : mp2) {
            if (i - j.second < 0) continue;
            if (dp[i - j.second] == k - 1) {
                i -= j.second;
                --k;
                out_noreturn(j.first);
                break;
            }
        }
    }
